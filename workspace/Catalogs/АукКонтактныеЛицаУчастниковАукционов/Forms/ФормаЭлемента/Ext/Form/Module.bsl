...
Функция ОбработчикКаналаВзаимодействияСуществует(Знач пКаналВзаимодействия)
	Возврат Справочники.АукКаналыВзаимодействия.ЕстьОбработчикКаналаВзаимодействия(пКаналВзаимодействия);	
КонецФункции	
...
Функция ПолноеОписаниеСвойствКаналаВзаимодействияНаСервере(Знач пКаналВзаимодействия)
	...
	Возврат Справочники.АукКаналыВзаимодействия.ПолноеОписаниеСвойствКаналаВзаимодействия(пКаналВзаимодействия);
...
Процедура ПроверитьДанныеФормыПередЗаписью(пОтказ)
	...
	EMailСсылка = АукОбщийВызовСервера.ЭлементНастроек_Email();
	...
	Если EMailСсылка <> Неопределено Тогда
		...
		Для Каждого СтрокаТч Из масСтрокиТч Цикл
			...
			Если Не АукОбщийКлиентСервер.EmailКорректный(СтрокаТч.ЗначениеНастройки) Тогда
...
Процедура УдалитьНастройкиКаналовВзаимодействияПоВидуКаналовВзаимодействия(пКаналВзаимодействия)
	АукОбщийКлиентСервер.УдалитьСтрокиТаблицыПоСтруктуреОтборов(Объект.НастройкиКаналовВзаимодействия, Новый Структура("КаналВзаимодействия", пКаналВзаимодействия));	
КонецПроцедуры
